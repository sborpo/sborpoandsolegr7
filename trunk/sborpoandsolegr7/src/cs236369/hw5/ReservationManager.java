...
public  class ReservationManager {
	...
	public static boolean areReservationsOverlap(Connection conn,String instId,TimeSlot start,int length) throws SQLException
	{
		TimeSlot end = TimeSlot.addTimeSlot(start, length);
		ResultSet set=null;
		long instIdNum= Long.valueOf(instId);
		try{
		String query= " SELECT * FROM reservations WHERE" +
				...
				" ((slotend<slotbegin) AND ((year+1)=?) AND (?<slotend))) AND (instId=?) ";//params : year, start.slotnum,start.slotunum case that -----start--------|year|----|meStart|--end
		...
		prepareStatement.setLong(17, instIdNum);
	...
	public static void makeReservation(String id, String slotYear,
		...
		try{
			...
			if (areReservationsOverlap(conn,id, new TimeSlot(year, num), length)) {
