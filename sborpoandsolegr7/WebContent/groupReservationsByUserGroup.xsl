...
<body>
  ...
  <table border="1">
    <xsl:for-each select="/Results/Row[1]/*">
       <xsl:if test="local-name() != 'groupId' "> <td>  <xsl:value-of select="local-name()"/> </td> </xsl:if> 
    </xsl:for-each>

     <xsl:for-each select="/Results/Row[groupId=$GROUP]">
      <tr>
      <xsl:for-each select="*"><xsl:if test="(local-name() != 'groupId' )"> <td>  <xsl:value-of select="."/> </td> </xsl:if>  </xsl:for-each>
